...
public interface UserRepository extends JpaRepository<User, Integer> {
	...
	@Query("select min(u.mealOrders.size) from User u")
	Double minOrdersPerUser();

	@Query("select max(u.mealOrders.size) from User u")
	Double maxOrdersPerUser();

	@Query("select avg(u.mealOrders.size) from User u")
	Double avgOrdersPerUser();
